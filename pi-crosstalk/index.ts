 ...
 * the `--crosstalk` flag, each pi session creates a control socket at
 * `~/.pi/crosstalk/<session-id>.sock` that accepts JSON-RPC commands.
 *
 * Features:
...
async function selectSummarizationModel(
	...
	if (codexModel) {
		...
		if (auth.ok && auth.apiKey) return codexModel;
	}

	const haikuModel = modelRegistry.find("anthropic", HAIKU_MODEL_ID);
	if (haikuModel) {
		const auth = await modelRegistry.getApiKeyAndHeaders(haikuModel);
		if (auth.ok && auth.apiKey) return haikuModel;
	}
...
function getInfoPath(sessionId: string): string {
	return path.join(CONTROL_DIR, `${sessionId}${INFO_SUFFIX}`);
}

async function readSessionInfo(sessionId: string): Promise<SessionInfoFile | null> {
	try {
		const raw = await fs.readFile(getInfoPath(sessionId), "utf-8");
		const parsed = JSON.parse(raw) as SessionInfoFile;
...
async function writeSessionInfo(ctx: ExtensionContext): Promise<void> {
	const sessionId = ctx.sessionManager.getSessionId();
	if (!isSafeSessionId(sessionId)) return;
	const payload: SessionInfoFile = {
		sessionId,
	...
	try {
		await fs.writeFile(getInfoPath(sessionId), JSON.stringify(payload), "utf-8");
	} catch {
		// ignore
	}
}
...
async function getLiveSessions(): Promise<LiveSessionInfo[]> {
	...
	for (const entry of entries) {
		...
		const info = await readSessionInfo(sessionId);
		const name = info?.name ?? aliases[0];
		const cwd = info?.cwd;
		sessions.push({ sessionId, name, cwd, aliases, socketPath });
	}
...
function handleSocketDataChunk(
	pi: ExtensionAPI,
	state: SocketState,
	socket: net.Socket,
...
function parseMentionInput(text: string): MentionParseResult | null {
	const trimmed = text.trimStart();
	if (!trimmed.startsWith("@@")) return null;

	const rest = trimmed.slice(2).trimStart();
	if (!rest) {
		return { targetToken: null, message: "", queryHint: "" };
	}
	...
	const targetToken = match[1]?.trim() || null;
	const message = match[2]?.trim() || "";
	return {
		targetToken,
		message,
		queryHint: targetToken ?? "",
	};
}

function normalizeTargetToken(raw: string): { kind: "session" | "name"; value: string } {
	const [prefix, ...restParts] = raw.split(":");
	const rest = restParts.join(":");

	if (!rest) {
		return { kind: "name", value: prefix };
	}

	if (prefix === "session" || prefix === "id") {
		return { kind: "session", value: rest };
	}
	if (prefix === "name") {
...
function formatSessionLabel(session: LiveSessionInfo): string {
	const cwd = session.cwd ?? "";
	const base = cwd ? path.basename(cwd) : "-";
	const name = session.name ?? session.aliases[0] ?? "-";
	return `${base} ${name} ${session.sessionId}`;
}

async function pickSessionWithFzf(sessions: LiveSessionInfo[], queryHint: string): Promise<LiveSessionInfo | null> {
	if (sessions.length === 0) return null;
	const lines = sessions.map((session) => {
		const cwd = session.cwd ?? "";
		const base = cwd ? path.basename(cwd) : "-";
		const name = session.name ?? session.aliases[0] ?? "-";
		const aliases = session.aliases.length ? session.aliases.join(",") : "-";
		return `${base}\t${name}\t${cwd}\t${session.sessionId}\t${aliases}`;
	});

	return new Promise((resolve) => {
		const args = ["--ansi", "--delimiter", "\t", "--with-nth", "1,2,3,4", "--nth", "1,2,3,4,5"];
		if (queryHint) {
			args.push("--query", queryHint);
		}
...
async function pickSessionWithUi(ctx: ExtensionContext, sessions: LiveSessionInfo[]): Promise<LiveSessionInfo | null> {
	const options = sessions.map((session) => formatSessionLabel(session));
	const selected = await ctx.ui.select("Select session", options);
	if (!selected) return null;
	return sessions.find((session) => formatSessionLabel(session) === selected) ?? null;
}

function resolveSessionByName(sessions: LiveSessionInfo[], name: string): LiveSessionInfo | null {
	const normalized = name.trim().toLowerCase();
	for (const session of sessions) {
		if (session.sessionId.toLowerCase() === normalized) return session;
		if (session.name?.toLowerCase() === normalized) return session;
		if (session.aliases.some((alias) => alias.toLowerCase() === normalized)) return session;
	}
	return null;
}

function resolveSessionById(sessions: LiveSessionInfo[], sessionId: string): LiveSessionInfo | null {
	const normalized = sessionId.trim().toLowerCase();
	return sessions.find((session) => session.sessionId.toLowerCase() === normalized) ?? null;
}

function formatMentionTarget(session: LiveSessionInfo): string {
	const name = session.name?.trim();
	if (name && isSafeAlias(name)) return name;
	return session.sessionId;
}

async function sendCrosstalkMessage(
	...
	const result = await sendRpcCommand(session.socketPath, sendCommand);
	if (!result.response.success) {
		if (ctx.hasUI) {
			ctx.ui.notify(`Crosstalk failed: ${result.response.error ?? "unknown error"}`, "error");
		}
		return false;
	}

	if (ctx.hasUI) {
		ctx.ui.notify(`Sent to ${formatMentionTarget(session)}`, "info");
	}
	return true;
}
...
// biome-ignore lint/complexity/noExcessiveCognitiveComplexity: legacy control flow
async function handleCommand(pi: ExtensionAPI, state: SocketState, command: RpcCommand, socket: net.Socket): Promise<void> {
	...
	if (command.type === "get_summary") {
		...
		if (!auth.ok || !auth.apiKey) {
			respond(false, "get_summary", undefined, auth.ok ? "No API key available for summarization model" : auth.error);
			return;
		}
...
async function createServer(pi: ExtensionAPI, state: SocketState, socketPath: string): Promise<net.Server> {
	const server = net.createServer((socket) => {
		...
		socket.on("data", (chunk) => handleSocketDataChunk(pi, state, socket, bufferRef, chunk));
	});
...
function handleRpcClientDataChunk(
	state: RpcClientState,
	chunk: string,
	command: RpcCommand,
	waitForEvent: RpcClientOptions["waitForEvent"],
	cleanup: () => void,
	socket: net.Socket,
	resolve: (value: { response: RpcResponse; event?: { message?: ExtractedMessage; turnIndex?: number } }) => void,
	...
	while (newlineIndex !== -1) {
		...
		try {
			...
			if (msg.type === "event" && msg.event === "turn_end" && waitForEvent === "turn_end") {
				...
				resolve({ response: state.response, event: msg.data });
				return;
			}
		} catch {
			// ignore parse errors
...
async function sendRpcCommand(
	...
	return new Promise((resolve, reject) => {
		...
		socket.on("data", (chunk) => {
			handleRpcClientDataChunk(state, chunk, command, waitForEvent, cleanup, socket, resolve, reject);
		});
...
export default function (pi: ExtensionAPI) {
	pi.registerFlag(CONTROL_FLAG, {
		description: "Enable per-session control socket under ~/.pi/crosstalk",
		type: "boolean",
	});
	...
	// biome-ignore lint/complexity/noExcessiveCognitiveComplexity: input routing logic
	pi.on("input", async (event, ctx) => {
		if (event.source === "extension") return { action: "continue" };

		const parsed = parseMentionInput(event.text);
		if (!parsed) return { action: "continue" };

		if (pi.getFlag(CONTROL_FLAG) !== true) {
			if (ctx.hasUI) {
				ctx.ui.notify("Crosstalk not enabled (use --crosstalk)", "warning");
			}
			return { action: "handled" };
		}

		if (!parsed.targetToken || parsed.message.length === 0) {
			if (!ctx.hasUI) {
				return { action: "handled" };
			}
			...
			if (sessions.length === 0) {
				ctx.ui.notify("No live sessions found.", "warning");
				return { action: "handled" };
			}

			let selected: LiveSessionInfo | null = null;
			try {
				selected = await pickSessionWithFzf(sessions, parsed.queryHint);
			} catch {
				selected = null;
			}
			if (!selected) {
				selected = await pickSessionWithUi(ctx, sessions);
			}
			if (!selected) return { action: "handled" };

			ctx.ui.setEditorText(`@@${formatMentionTarget(selected)} `);
			return { action: "handled" };
		}

		const target = normalizeTargetToken(parsed.targetToken);
		const sessions = await getLiveSessions();
		const session =
			target.kind === "session" ? resolveSessionById(sessions, target.value) : resolveSessionByName(sessions, target.value);

		if (!session) {
			if (ctx.hasUI) {
				ctx.ui.notify(`Unknown session: ${parsed.targetToken}`, "warning");
			}
			return { action: "handled" };
		}

		if (session.sessionId === ctx.sessionManager.getSessionId()) {
			if (ctx.hasUI) {
		...
		await sendCrosstalkMessage(session, parsed.message, state, ctx);
		return { action: "handled" };
	});
...
Messages automatically include sender session info for replies. When you want a response, instruct the target session to reply directly to the sender by calling send_to_session with the sender_info reference (do not poll get_message).`,
		...
		// biome-ignore lint/complexity/noExcessiveCognitiveComplexity: legacy tool handler
		...
		// biome-ignore lint/complexity/noExcessiveCognitiveComplexity: renderer branching
...
function registerListSessionsTool(pi: ExtensionAPI): void {
	pi.registerTool({
		name: "list_sessions",
		label: "List Sessions",
		description: "List live sessions that expose a crosstalk socket (optionally with session names).",
		parameters: Type.Object({}),
		async execute() {
			const sessions = await getLiveSessions();
...
function createSessionsCommandHandler(pi: ExtensionAPI, customType: string) {
...
function registerControlSessionsCommand(pi: ExtensionAPI): void {
	pi.registerCommand("crosstalk-sessions", {
		description: "List controllable sessions (from crosstalk sockets)",
		handler: createSessionsCommandHandler(pi, "crosstalk-sessions"),
	});
	pi.registerCommand("control-sessions", {
		description: "Alias for /crosstalk-sessions",
		handler: createSessionsCommandHandler(pi, "crosstalk-sessions"),
	});
}
