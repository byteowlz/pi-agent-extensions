...
function createChildSessionFile(parentSessionFile: string, sessionDir: string, cwd: string): string | null {
	try {
...
function runPiTask(input: DelegateTaskInput, parentSessionFile: string | null): Promise<DelegateTaskResult> {
	return new Promise((resolve) => {
		const id = randomUUID().slice(0, 8);
		const cwd = input.cwd ?? process.cwd();
		const sessionDir = parentSessionFile ? path.dirname(parentSessionFile) : undefined;
		const sessionFile = parentSessionFile && sessionDir ? createChildSessionFile(parentSessionFile, sessionDir, cwd) : null;
...
export default function registerDelegateExtension(pi: ExtensionAPI): void {
	...
	pi.registerTool({
		...
		async execute(_id, params, _onUpdate, ctx) {
			...
			const results = await Promise.all(tasks.map((task) => runPiTask(task, parentSessionFile)));
			const successCount = results.filter((r) => r.status === "completed").length;
			return {
				content: [
					{
						type: "text",
						text: `${successCount}/${results.length} completed${results.length === 1 ? `\n\n${results[0].output}` : ""}`,
					},
				],
				details: { results },
			};
		...
		renderCall(args, theme) {
			...
			const isParallel = taskCount > 0;
			const asyncLabel = args.async ? theme.fg("warning", " [async]") : "";
			if (isParallel) {
				return new Text(`${theme.fg("toolTitle", theme.bold("delegate "))}parallel (${taskCount})${asyncLabel}`, 0, 0);
			}
			return new Text(
		...
		renderResult(result, _options, theme) {
			const details = result.details as { results?: DelegateTaskResult[] } | undefined;
			if (details?.results) {
				const lines = details.results.map((res) => {
