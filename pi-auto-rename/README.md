...
- **Agent rename tool**: The agent can rename the session via a `rename_session` tool (title sans readable id; the suffix is generated automatically)
- **Fork-safe readable ids**: The readable-id suffix is regenerated from the forked session's id on fork, so each branch gets a distinct stable id
...
### Agent Tool: `rename_session`

The extension registers a `rename_session` tool that the agent may call to set the
session's display name. The agent provides only a concise title; the readable-id
suffix is owned and generated by this extension and is appended automatically when
`readableIdSuffix` is enabled (and never duplicated).

Could be used, for example, when the agent has implemented a meaningful change or the
session's focus has shifted:
...
When a session is forked, the new session inherits the parent's title but must carry
a distinct stable readable id. The extension listens for `session_start` with reason
`fork` and regenerates the readable-id suffix from the forked session's id. An
environment override (`readableIdEnv`) is session-independent and is preserved as-is.
